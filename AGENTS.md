...
- Android renders a native app-settings safety button over the WebView, hidden only while the native settings screen is open. Keep this path independent of Hermes WebUI page health so users can recover from an unreachable or misconfigured server by switching servers in native settings.
- The injected Hermes WebUI "Application Settings" entry should anchor immediately after the WebUI Settings item when present, with Help only as a fallback anchor for older or changed sidebar markup.
