...
- Manual runs of the orchestration workflow auto-bump `appVersionName` from the latest published GitHub release tag (`vX.Y.Z -> vX.Y.(Z+1)`) before building. Gradle derives `versionCode` directly from semantic version (`major*10000 + minor*100 + patch`) so release numbering stays monotonic without a separate manual versionCode edit.
