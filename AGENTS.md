...
- The Issue #65/#92 Clarify keyboard shim suppresses only automatic focus on `#clarifyInput`. Direct pointer/touch input, keyboard Tab navigation, and the Clarify **Other** action must still focus that field. Unrelated dialogs—including the workspace new-folder name input from Issue #90—must never be scanned, blurred, or have `tabindex` rewritten; do not restore blanket modal-input handling.
