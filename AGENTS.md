...
- The Issue #65 dialog keyboard shim suppresses only programmatic input focus that would open the soft keyboard over a Clarify prompt. Explicit pointer/touch interaction must always be allowed to focus editable dialog fields, including the workspace new-folder name input (Issue #90); do not restore blanket blurring of all modal inputs.
