...
| BUG-025 | 2026-06-27 | Settings | Added a native app-settings safety button over the WebView so users can reopen Android settings and switch servers even when the configured Hermes server is unreachable or WebUI never renders; the injected WebUI "Application Settings" entry now anchors after the regular WebUI Settings item, with Help only as a fallback |
